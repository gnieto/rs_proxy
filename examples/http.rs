...
use rs_proxy::proxy::RoundRobin;

use rs_proxy::connection::http::{HttpConnection, NoopProxy, LoggerProxy, AddHeaderProxy};
...
struct MyHandler {
    ...
    tokens: BitSet,
    round_robin: RoundRobin<SocketAddr>,
}

impl MyHandler {
    pub fn new() -> Self {
        let servers = vec!["127.0.0.1:8080", "127.0.0.1:8081"];
        let mut parsed = Vec::new();

        for s in servers {
            let sockaddr = s.parse().unwrap();
            parsed.push(sockaddr);
        }
        ...
        MyHandler {
            ...
            timers: HashMap::new(),
            round_robin: RoundRobin::new(parsed),
        }
    }
    ...
    pub fn handle_accept(&mut self, event_loop: &mut EventLoop<MyHandler>, token: Token) -> Result<(Token, Token), &str> {
        ...
        let downstream = HttpConnection::new(downstream, LoggerProxy);
